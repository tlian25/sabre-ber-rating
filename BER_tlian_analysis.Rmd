...
lag_table = matrix(NA, nrow = nrow(pt), ncol = 7)
j = 1
for (b in batters) {
  ...
  if (dim(b.subset)[1] >= 4) {
    for (i in 4:dim(b.subset)[1]) {
      temp = c(b.subset$res_batter[i], b.subset$player_name[i],  
               b.subset$season[i], b.subset[(i-3):i, ]$BER)
      lag_table[j,] = temp
      j = j + 1
    }
...
colnames(lag_table) = c("res_batter", "player_name", "season",
                        "BER", "BER_L1", "BER_L2", "BER_L3")

lag_table[, c("season", "BER", "BER_L1", "BER_L2", "BER_L3")] =
  lapply(lag_table[, c("season", "BER_L3", "BER_L2", "BER_L1", "BER")], 
         as.numeric)
...
# Regression
lag_reg = lm(BER ~ BER_L1 + BER_L2 + BER_L3, data = lag_table)
